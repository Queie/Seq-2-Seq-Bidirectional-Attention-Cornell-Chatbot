...
def zero_pad(qtokenized, atokenized, w2idx):
    ...
    for i in range(data_len):
        q_indices = pad_seq(qtokenized[i], w2idx)
        a_indices = pad_seq(atokenized[i], w2idx)
        idx_q[i] = np.array(q_indices)
        idx_a[i] = np.array(a_indices)

    return idx_q, idx_a

def zero_pad_array(qtokenized, atokenized, w2idx):
    # num of rows
    data_len = len(qtokenized)
    ...
    for i in range(data_len):
        q_indices = pad_seq(qtokenized[i], w2idx)
        a_indices = pad_seq(atokenized[i], w2idx)
        idx_q.append(q_indices)
        idx_a.append(a_indices)
...
def process_data():
    ...
    print('\n >> Zero Padding')
    idx_q, idx_a = zero_pad_array(qtokenized, atokenized, w2idx)
    print idx_q[:5]
...
def load_data_as_array(PATH=''):
    id2line = get_id2line(PATH)
    convs = get_conversations(PATH)
    questions, answers = gather_dataset(convs, id2line)
    questions = [line.lower() for line in questions]
    answers = [line.lower() for line in answers]
    questions = [filter_line(line, EN_WHITELIST) for line in questions]
